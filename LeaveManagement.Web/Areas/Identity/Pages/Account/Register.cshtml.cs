    ...
    public class RegisterModel : PageModel
    {
        ...
        public async Task<IActionResult> OnPostAsync(string returnUrl = null)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                //set assign values based on the properties we added in InputModel
                user.Firstname = Input.Firstname;
                user.Lastname= Input.Lastname;
                user.DateJoined = Input.DateJoined ?? default; // if it is null give a default value
                user.DateOfBirth = Input.DateOfBirth ?? default;

                var result = await _userManager.CreateAsync(user, Input.Password);//hashes password in backgrnd and store it

                if (result.Succeeded)
                {
                    _logger.LogInformation("User created a new account with password.");

                    //Give user's the 'User' role
